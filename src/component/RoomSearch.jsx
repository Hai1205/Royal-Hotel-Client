...
import { getAvailableRoomsByDateAndType, getRoomTypes } from '../utils/api/roomApi';

const RoomSearch = ({ handleSearchResult }) => {
  const [startDate, setStartDate] = useState(null);
  const [endDate, setEndDate] = useState(null);
  const [roomType, setRoomType] = useState('');
  const [roomTypes, setRoomTypes] = useState([]);  // Initialize as an empty array
  ...
  const handleInternalSearch = async () => {
    ...
    try {
      ...
      showError("Unknown error occurred: " + error.message);
    }
  };
